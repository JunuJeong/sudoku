...
class solver():
    def __init__(self, file=None, max_step=100):
    ...
    def run(self, table=[], notes=[], max_step=0, log=True):
        if table == []:
            table = self.table
        if notes == []:
            notes = self.notes
        ...
        is_solved, is_collision = False, False

        if self.step >= max_step:
            if self.depth == 0:
                self.log(log, 'not-solved', self.depth)
            return is_solved, is_collision
        ...
        for step in itertools.count():
            is_solved, is_updated, is_collision = self.solve(table, notes)
            complexity = self.complexity(notes)
            self.step += 1
            self.log(log, 'update', self.depth, table, complexity)
            ...
            if not is_updated and not is_solved:
                ...
                for trial_notes in trials_notes:
                    is_solved, is_collision = self.run(
    ...
    def solve(self, table, notes):
        ...
        is_collision = self.is_collision(table, notes)
        return is_solved, is_updated, is_collision
    ...
    def is_collision(self, table, notes):
